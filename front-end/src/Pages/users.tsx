      ...
      <UITemplate>
        <>
          <div className="overscroll-none flex flex-col h-screen items-center w-full sm:w-1/4 bg-secondary top-0">
            <Login />
            {user.map((user) => (
              <div className="bg-primary p-8 rounded-xl m-2 w-5/6 h-fit flex flex-col text-nowrap" onClick={() => navigate(`/users/${user.ID}`)}>
          ...
          <div className="bg-primary flex flex-1 overflow-auto"><Outlet /></div>
        </>
      </UITemplate>
    </>
  )
...
function UserProfile() {
  ...
  return (
    <div className="flex flex-col grow m-4 items-center">
      ...
      <img className="size-1/3 rounded-xl shadow object-cover" src="https://media.istockphoto.com/id/1687018104/vector/vector-flat-illustration-in-grayscale-avatar-user-profile-person-icon-gender-neutral.jpg?s=612x612&w=0&k=20&c=PDi0AqXTtZ6d2Y7ahkMJEraVrC_fYCvx0HW508OWg-4=" alt="" />
      <Posts user_id={id} />
    </div>
  )
}
